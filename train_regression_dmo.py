...
dmo_data, dmo_labels = pdd.get_patient_data(PatientDataType.DMO, ids=ids, static_features=static_features)
...
lstm_train = LSTMRegressionTrain(
    model=model, optimiser=optimiser, device=device, config=config, verbose=False
)
